<template>
  <div id="article">
    <div class="nav">
		...
		<div class="content">
			<div class="header clear"><h2><img :src="articleData.author_face" alt=""/></h2><p>新闻主要内容</p></div>
		    <div class="cont">
		    	<h3>{{articleData.title}}</h3>
            <!-- 从后台取出的时间一般为一个时间戳，所以需要自定义过滤器normalTime来过滤变为正常的时间 -->
...
<script>
  export default{
    ...
    mounted(){
      /*
        如何获取路由上面的数据？
        Vue实例上面有一个$route,表示当前的路由对象，$route.path表示当前路由路径
       */
      //var path=this.$route.path;
      console.log(this.$route.path);
      var reg=/\/article\/(\d+)/;
      //alert(this.$route.path.match(reg))
